...
- No automated backups for n8n PostgreSQL database
- No automated backups for Chatwoot PostgreSQL
- WAHA session data in Docker volume (no backup)
...
1. **4GB RAM, no swap** — OOM kills are real. Always set memory limits on services.
2. **Single server** — all services on one instance means resource contention is frequent.
...
9. **PostgreSQL 12 EOL** — Chatwoot's PostgreSQL is end-of-life. Upgrade needed.
